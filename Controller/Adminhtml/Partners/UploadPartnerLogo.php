 ...
 */
class UploadPartnerLogo extends \Magento\Backend\App\Action
{
    /**
     * @var Filesystem
     */
	...
	public function execute()
    {
        try {
            ...
            $uploaderFactory = $this->uploaderFactory->create(['fileId' => PartnersInterface::PARTNER_LOGO]);
            $uploaderFactory->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
            $uploaderFactory->setAllowRenameFiles(true);
            $uploaderFactory->setFilesDispersion(true);
            $mediaDirectory = $this->fileSystem->getDirectoryRead(DirectoryList::MEDIA);
            $destinationPath = $mediaDirectory->getAbsolutePath(PartnersInterface::PARTNER_LOGO_UPLOAD_DIR);
            $result = $uploaderFactory->save($destinationPath);

            unset($result['tmp_name']);
            unset($result['path']);

            $result['url'] = $this->getBaseMediaUrl() . PartnersInterface::PARTNER_LOGO_UPLOAD_DIR . $result['file'];
